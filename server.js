...
.catch(err => {
	...
	app.get('/player_info', function(req, res) {
		var query = 'SELECT * FROM football_players order by football_players.id';
		db.any(query)
	 .then(function (rows) {
	...
	app.get('/player_info/select_player', function(req, res) {
		var selected_id = parseInt(req.query.player_choice || 0);
		var queryx = 'SELECT * FROM football_players order by football_players.id';
		var queryy = 'SELECT * FROM football_players where football_players.id = ' + selected_id + ';';
		var queryz = 'select count(*) as games_played FROM football_games where '+selected_id+' = ANY(football_games.players);';
		console.log("====================== " + queryy);
		db.task('get-everything', task => {
    return task.batch([
        task.any(queryx),
